...
- **The reranker was dead in production.** ~~`mcp-server.ts:57` hardcodes `rerank:"none"`.~~ (v0.7.3 enabled it on the legacy MemoryRetriever branch. v0.7.4 completes the fix by passing `MEMEX_RERANK_*` into the document-aware `UnifiedRetriever` production path.)
...
| C5 | MCP memory_recall disables reranker (NO-OP for reranker-dependent design) | IMPLEMENTED through v0.7.4. v0.7.3 enabled the legacy branch; v0.7.4 wires the document-aware UnifiedRetriever and enforces the post-rerank floor. |
