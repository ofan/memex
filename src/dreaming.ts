      ...
      if (line.length >= 20) {
        ...
        if (embedder) {
          try {
            ...
            log(logPath, "dream:reflect", {
              semantic_dedup_error: (err as Error)?.message ?? String(err),
            });
          }
        }

        if (isSemanticDupe) continue;
...
export interface DreamConfig {
  ...
  /** Embedder for semantic dedup of reflection learnings. If omitted, dedup is
   *  exact-text only (same as pre-v0.7 behavior). */
  embedder?: Embedder;
}
