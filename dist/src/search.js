...
export function getDefaultDbPath(indexName = "index") {
    ...
    try {
        mkdirSync(dbCacheDir, { recursive: true });
    }
    catch { }
...
function initializeDatabase(db) {
  ...
  `);
    ...
    const sectionCount = db.prepare(`SELECT COUNT(*) as c FROM document_sections`).get().c;
    const activeDocCount = db.prepare(`SELECT COUNT(*) as c FROM documents WHERE active = 1`).get().c;
...
export function insertDocument(db, collectionName, path, title, hash, createdAt, modifiedAt) {
  ...
  `).run(collectionName, path, title, hash, createdAt, modifiedAt);
    ...
    const doc = db.prepare(`SELECT id FROM documents WHERE collection = ? AND path = ? AND active = 1`).get(collectionName, path);
    if (doc) {
        removeSectionsFTS(db, doc.id);
        const contentRow = db.prepare(`SELECT doc FROM content WHERE hash = ?`).get(hash);
        if (contentRow) {
...
export function updateDocument(db, documentId, title, hash, modifiedAt) {
    ...
    const docRow = db.prepare(`SELECT d.collection, d.path, c.doc FROM documents d JOIN content c ON c.hash = d.hash WHERE d.id = ?`).get(documentId);
    if (docRow) {
 ...
 */
export function populateSectionsFTS(db, documentId, filepath, content) {
    const sections = splitSections(content);
    const insertSection = db.prepare(`INSERT INTO document_sections (document_id, section_idx, char_pos, heading)
     VALUES (?, ?, ?, ?)`);
    const insertFts = db.prepare(`INSERT INTO sections_fts (rowid, filepath, heading, body)
     VALUES (?, ?, ?, ?)`);
 ...
 */
export function removeSectionsFTS(db, documentId) {
    const rows = db.prepare(`SELECT id FROM document_sections WHERE document_id = ?`).all(documentId);
    if (rows.length > 0) {
 ...
 */
export function splitSections(content, options) {
    if (!content)
        return [];
