 ...
 */
export async function indexPath(db, pathConfig) {
    const result = {
        ...
        for (const relativeFile of files) {
            try {
                ...
                if (existing) {
                    ...
                    insertContent(db, hash, content, now);
                    const stat = statSync(filepath);
                    insertDocument(db, pathConfig.name, path, title, hash, stat ? new Date(stat.birthtime).toISOString() : now, stat ? new Date(stat.mtime).toISOString() : now);
                    result.indexed++;
                }
            }
            catch (err) {
 ...
 */
export async function indexAllPaths(db, paths) {
    // Clean up stale collections not in current config
    const activeCollections = new Set(paths.map(p => p.name));
    const dbCollections = db.prepare(`SELECT DISTINCT collection FROM documents WHERE active = 1`).all();
    for (const { collection } of dbCollections) {
        if (!activeCollections.has(collection)) {
            const stale = db.prepare(`UPDATE documents SET active = 0 WHERE collection = ? AND active = 1`).run(collection);
            if (stale.changes > 0) {
                console.warn(`doc-indexer: deactivated ${stale.changes} documents from stale collection "${collection}"`);
            }
        }
    }
    const results = [];
 ...
 */
export async function embedDocuments(db, dimensions, embedder) {
    const result = { embedded: 0, chunks: 0, errors: [] };
 ...
 */
async function embedDocumentsViaEmbedder(db, dimensions, embedder, hashesToEmbed, result) {
    ...
    for (const item of hashesToEmbed) {
        try {
            ...
            try {
                db.prepare(`INSERT OR IGNORE INTO content_vectors (hash, seq, pos, model, embedded_at) VALUES (?, 0, 0, '_error', ?)`).run(item.hash, now());
            }
