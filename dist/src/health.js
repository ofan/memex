...
export function aggregateHealthStatus(checks) {
...
export function filterMemexLogLines(rawLog, limit = 100) {
    const truncateLine = (line, maxChars = 240) => line.length > maxChars ? `${line.slice(0, maxChars - 1)}…` : line;
    const lines = rawLog
        .split(/\r?\n/)
        .map(line => line.trim())
        .filter(Boolean)
...
export async function getLatestOpenClawLogPath(logDir = "/tmp/openclaw") {
    try {
...
export function buildAuditPrompt(snapshot, logLines) {
...
function flattenText(value) {
    ...
    if (value && typeof value === "object") {
        ...
        if ("parts" in record)
            return flattenText(record.parts);
    }
    return "";
}
export function extractAuditConclusion(messages) {
