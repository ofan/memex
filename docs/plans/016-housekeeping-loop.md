...
- [x] **M1.4** PR #73 squash-merged into main (sha `222060e`). Repo policy forbids merge commits, so squash was the right strategy. Required two extra steps not in the original plan: (a) push to main rejected by branch protection → routed through PR #73; (b) PR initially had conflicts since GitHub uses default merge strategy not `-X theirs` → resolved by merging origin/main into memex-v0.7 with `-X ours` first, then PR was clean. Main is now at v0.7.1's state. The 2 dependabot UI alerts (picomatch high + medium) should auto-close on dependabot's next scan.
...
- [x] **M4.1** User chose option (c) — delete outright. Branch deleted on 2026-05-11. Tip was `1fe98ed docs: rename remaining memory-unified references to memclaw`. Recoverable from GitHub reflog for ~30 days if needed.
