...
function formatCurrentTimeLine(nowMs, timezone) {
...
function ensureHint(text, hint) {
    return text.includes(hint) ? text : `${text}\n\n${hint}`.trim();
}
function ensureSafetyHints(text) {
    let next = text.trim();
...
export function buildMemoryFlushPlan(params = {}) {
    ...
    const forceFlushTranscriptBytes = parseByteSize(defaults?.forceFlushTranscriptBytes) ?? DEFAULT_MEMORY_FLUSH_FORCE_TRANSCRIPT_BYTES;
    const reserveTokensFloor = normalizeNonNegativeInt(cfg?.agents?.defaults?.compaction?.reserveTokensFloor, DEFAULT_MEMORY_FLUSH_RESERVE_TOKENS_FLOOR);
...
//# sourceMappingURL=flush-plan.js.map