...
export function parseRerankResponse(provider, data) {
...
export class MemoryRetriever {
    ...
    async fuseResults(vectorResults, bm25Results) {
        ...
        for (const id of allIds) {
            ...
            if (this.config.fusionMethod === "zscore") {
                ...
                if (vectorResult && bm25Result) {
                    fusedScore = clamp01(this.config.vectorWeight * vectorScore + this.config.bm25Weight * bm25Score, 0.1);
                }
     ...
     */
    applyRecentlyRecalledPenalty(results, recentlyRecalled) {
        if (!recentlyRecalled || recentlyRecalled.size === 0)
            return results;
