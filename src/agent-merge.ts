 ...
 */
export function mergeAgentLists(unified: unknown, legacy: unknown): string[] | undefined {
  const u = Array.isArray(unified) ? (unified as string[]) : [];
  const l = Array.isArray(legacy) ? (legacy as string[]) : [];
  if (u.length === 0 && l.length === 0) return undefined;
