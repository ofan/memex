...
export function createMemexMcpServer(options) {
    ...
    server.registerTool("memory_store", {
        ...
        const { text, category, importance = 0.7, scope = "global", agent_id, session_id, device_id } = _params;
        if (isNoise(text)) {
        ...
        const clientName = detectClientName(_extra?._meta);
    ...
    function detectClientName(meta) {
    ...
    server.registerTool("memory_recall", {
        ...
        if (agent_id || session_id) {
            const derivResult = deriveScopes({
                cwd: process.cwd(),
                env: process.env,
                clientName: detectClientName(_extra?._meta),
            ...
            if (effectiveScopes) {
                ...
                if (agent_id) {
                    ...
                    if (!effectiveScopes.includes(agentTag))
                        effectiveScopes.push(agentTag);
                }
                if (session_id) {
                    const sessionTag = derivResult.tags.find(t => t.startsWith("session:"));
                    if (sessionTag && !effectiveScopes.includes(sessionTag))
                        effectiveScopes.push(sessionTag);
                }
            }
            else {
    ...
    server.registerTool("memory_stats", {
        ...
        const scopeRows = db.prepare("SELECT scope, COUNT(*) as cnt FROM memory_scopes GROUP BY scope ORDER BY cnt DESC").all();
        const byScope = {};
