 ...
 */
async function lenientFetch(input, init) {
    try {
    ...
    return new Promise((resolve, reject) => {
        ...
        const socket = netConnect({ host: url.hostname, port: parseInt(url.port || "80") }, () => {
        ...
        if (init?.signal)
            init.signal.addEventListener("abort", () => socket.destroy());
    });
}
            ...
            if (isContextError && this._autoChunk) {
                try {
                    ...
                    const chunkResults = await Promise.all(validTexts.map(async (text, idx) => {
                        ...
                        const embeddings = await Promise.all(chunkResult.chunks.map((chunk) => this.embedSingle(chunk, task)));
