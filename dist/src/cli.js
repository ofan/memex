...
import { inspectVecTable, assertVecShadowIntegrity, needsMaintenance, populateManifestFromCurrentState, dropAndRecreateVecTable, readManifestSummary, reconcileManifestWithVecTable, VEC_TABLE, } from "./vec-table.js";
// ============================================================================
// Utility Functions
// ============================================================================
...
export function registerMemoryCLI(program, context) {
            ...
            if (options.json) {
                ...
                if (results.length === 0) {
                    ...
                    results.forEach((result, i) => {
                        ...
                        console.log(`${i + 1}. [${String(id).slice(0, 8)}] ${tag} ${text.slice(0, 120)} ` +
                            `(${(score * 100).toFixed(0)}%)`);
                    });
                }
            }
    ...
    memory
        ...
        try {
            ...
            if (doMemories) {
                ...
                console.log("\n── Memories ──");
                const memCount = await context.store.reEmbedMemories(currentModel, async (texts) => context.embedder.embedBatchPassage(texts), 20, (done, total) => {
    ...
    memory
        ...
        .description(`DESTRUCTIVE maintenance: replace the ${VEC_TABLE} ANN index with one sized for ` +
        ...
        try {
            ...
            const docDb = context.searchDb ?? context.store.db;
            ...
            if (doDocs && status.exists)
                clearDocEmbeddings(docDb);
            if (doMemories) {
                console.log("\n── memories ──");
                const n = await context.store.reEmbedMemories(embedder.model, async (texts) => embedder.embedBatchPassage(texts), 16, (done, total) => { if (done % 25 === 0 || done === total)
    ...
    memory
        ...
        try {
          ...
          `).all();
            ...
            let diskBytes = 0;
            if (context.searchDb && context.searchDb.name) {
                try {
                    diskBytes = statSync(context.searchDb.name).size;
                }
                catch { /* ignore */ }
            }
            const summary = {
            ...
            if (options.json) {
                ...
                if (docStats) {
                    ...
                    if (context.searchDb && context.searchDb.name) {
                        try {
                            const dbSize = statSync(context.searchDb.name).size;
        ...
        try {
            ...
            if (options.fresh) {
                ...
                if (sessionMemories.length > 0) {
                    ...
                    for (const m of sessionMemories) {
                        if (await context.store.delete(m.id))
                            deleted++;
                    }
            ...
            if (options.llmExtract) {
                if (!context.generationConfig) {
                    ...
                    const caps = await probeBackend(context.generationConfig.baseURL, context.generationConfig.model, context.generationConfig.apiKey);
    ...
    memory
        ...
        if (opts.dryRun) {
            ...
            const neverRecalled = db.prepare("SELECT COUNT(*) as c FROM memories WHERE (recall_count IS NULL OR recall_count = 0) AND timestamp < ?").get(Date.now() - 30 * 86400_000).c;
