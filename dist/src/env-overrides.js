 ...
 */
export function syncDebugEnvFromConfig(config, env = process.env) {
    if (env.MEMEX_DEBUG_RECALL !== undefined && env.MEMEX_DEBUG_RECALL !== "")
        return;
...
const VALID_RERANK_PROVIDERS = new Set([
 ...
 */
export function resolveCrossRerankerFromEnv(env = process.env) {
    ...
    const provider = requestedProvider === "jina"
        ? "jina"
        : requestedProvider && VALID_RERANK_PROVIDERS.has(requestedProvider)
            ? requestedProvider
            : "jina";
