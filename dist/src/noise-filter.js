 ...
 */
export function isStructuralNoise(text) {
    const s = text.trim();
    // Too short or too long
    const hasCJK = /[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]/.test(s);
    if (s.length < (hasCJK ? 4 : 10) || s.length > 2000)
        return true;
    ...
    if (s.startsWith("[Thread starter"))
        return true;
    ...
    if (/^(\[tool_result\]|<tool_result>|<function_result>|Tool output:)/im.test(s))
        return true;
    const lines = s.split("\n");
    // Code-heavy content: >50% of lines are inside fenced code blocks
    {
        ...
        if (lines.length > 3 && codeLines / lines.length > 0.5)
            return true;
    }
    // Stack traces: multiple lines matching stack frame patterns
    {
        ...
        if (pythonTrace.test(s) || javaTrace.test(s))
            return true;
    ...
    if (/(?<!\S)[A-Za-z0-9+/]{100,}={0,2}(?!\S)/.test(s))
        return true;
 ...
 */
export function identifyNoiseEntries(entries) {
    const noise = [];
 ...
 */
export function extractHumanText(text) {
    const s = text.trim();
    if (!s)
        return null;
