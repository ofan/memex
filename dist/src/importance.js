...
export function heuristicImportance(text) {
    const matchCount = MEMORY_TRIGGERS.filter(r => r.test(text)).length;
    if (matchCount === 0)
        return 0.3; // baseline — might still be useful context
    if (matchCount === 1)
        return 0.6;
    if (matchCount === 2)
        return 0.8;
 ...
 */
export async function scoreImportance(texts, endpoint, model, apiKey) {
