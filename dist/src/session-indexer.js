 ...
 */
export async function probeBackend(baseURL, model, apiKey) {
    const headers = {
    ...
    try {
        ...
        if (modelsResp.ok) {
            ...
            const modelInfo = models.find((m) => m.id === model || m.id === `models/${model}` || m.id?.endsWith(`/${model}`));
...
export function applyBackendCapabilities(config, caps) {
    return {
        ...config,
        ...
        maxWindowTokens: config.maxWindowTokens ?? (caps.contextWindow
 ...
 */
function parseCategoryPrefix(text) {
...
function detectCategoryHeuristic(text) {
    const lower = text.toLowerCase();
    if (/prefer|like|love|hate|want|ban|never|always/i.test(lower))
        return "preference";
    if (/decided|will use|switch(ed)? to|going forward|from now on|on hold|paused/i.test(lower))
        return "decision";
    if (/\+\d{10,}|@[\w.-]+\.\w+|is called/i.test(lower))
        return "entity";
    if (/\b(is|are|has|have|port|endpoint|expire|version|config)\b/i.test(lower))
        return "fact";
    return "other";
}
// ============================================================================
...
function estimateSessionTokens(turns) {
 ...
 */
export function binPackSessions(sessions, maxTokens) {
    const windows = [];
    let currentWindow = [];
    let currentTokens = 0;
...
export async function extractKnowledge(turns, config) {
    ...
    async function processWindow(wi) {
...
// Session Indexer
// ============================================================================
export async function indexSessions(store, embedder, config = {}) {
    ...
    if (cfg.rerankEndpoint && cfg.rerankModel) {
        try {
            importanceScores = await scoreImportance(truncated.map(t => t.text), cfg.rerankEndpoint, cfg.rerankModel);
        }
        catch {
    ...
    if (cfg.dryRun) {
        ...
        if (toIndex.length > 0) {
            ...
            if (toIndex.length > 30)
                console.warn(`  ... and ${toIndex.length - 30} more`);
    ...
    if (cfg.llmExtraction && toIndex.length > 1) {
        const keep = new Array(toIndex.length).fill(true);
    ...
    if (dedupedIndex.length > 0) {
        const storeKeep = new Array(dedupedIndex.length).fill(true);
    ...
    const entries = dedupedIndex.map((item, i) => {
...
export function listSessions(sessionsDir, opts) {
