# Memory Extraction Model Comparison — Apple Silicon
...
>
> **Hardware:** A capable ARM host (e.g., Apple Silicon base chip, 16 GB unified memory, 120 GB/s bandwidth). ~8 GB VRAM headroom after existing models (Qwen3-Embedding-0.6B + bge-reranker-v2-m3 + Qwen3-0.6B-Instruct ≈ 3.5 GB).
>
...
Apple Silicon base chip: **120 GB/s** (vs 273 GB/s M4 Pro, 410 GB/s M4 Max).
...
Phi-4-mini is actually 3.8B parameters — Microsoft's "mini" label is relative to Phi-4 (14B), not an actual small model. Benchmarks show ~65–75 tok/s on an M4 Pro chip with Ollama (a faster chip variant), implying ~30–38 tok/s on an M4 base chip. At ~1 second per extraction, it's 2–5× too slow for the hot path and uses 3.2 GB of precious VRAM.
...
*Report generated: 2026-03-08. LLM benchmarks are hardware- and version-sensitive; run `llama-bench` on your specific hardware to confirm latency numbers before committing.*
