...
import { identifyNoiseEntries } from "./src/noise-filter.js";
...
const memoryUnifiedPlugin = {
    ...
    register(api) {
        ...
        try {
            ...
            api.logger.warn(`memex: storage path issue — ${String(err)}\n` +
                `  The plugin will still attempt to start, but writes may fail.`);
        }
        const vectorDim = getVectorDimensions(config.embedding.model || "text-embedding-3-small", config.embedding.dimensions);
            ...
            (async () => {
                try {
                    ...
                    if (!probe.success) {
                        ...
                        api.logger.warn(`memex: dimension mismatch! Config expects ${vectorDim}d but model returns ${probe.dimensions}d. ` +
        ...
        // ========================================================================
        let searchStoreRef = new Proxy({}, {
            get(_, prop) {
                return _searchStore ? _searchStore[prop] : undefined;
            },
        });
        ...
        const workspaceToCollection = new Map();
        ...
        if (defaultDocPaths.length === 0) {
            const defaultWorkspace = api.config?.agents?.defaults?.workspace;
        ...
        const explicitPaths = config.documents?.paths?.map((p) => ({
            path: p.path,
            name: p.name,
            pattern: p.pattern,
        })) || [];
        ...
        if (config.documents?.enabled !== false && docPaths.length > 0) {
            try {
                ...
                if (!isCli) {
                    ...
                    const runDocIndex = async (silent = false) => {
                        try {
                            ...
                            const totals = indexResults.reduce((acc, r) => ({
                            ...
                            if (!silent && (totals.indexed > 0 || totals.updated > 0)) {
                                api.logger.info(`memex: indexed ${totals.indexed} new, ${totals.updated} updated, ${totals.unchanged} unchanged, ${totals.removed} removed docs`);
                            }
                            const backlog = getEmbeddingBacklog(searchDb);
                            if (backlog > 0) {
                                if (!silent)
                                    api.logger.info(`memex: embedding ${backlog} document hashes...`);
                                const embedResult = await embedDocuments(searchDb, searchDims, embedder);
                                if (!silent) {
                                    api.logger.info(`memex: embedded ${embedResult.embedded} docs (${embedResult.chunks} chunks)${embedResult.errors.length > 0 ? `, ${embedResult.errors.length} errors` : ""}`);
                                }
                            }
                        }
                        catch (err) {
                ...
                api.logger.info(`memex: document search enabled (db: ${unifiedDbFile}, paths: ${docPaths.map(p => p.name).join(", ")})`);
            }
            catch (err) {
        ...
        const documentSearchFn = async (query, queryVec, limit, collection) => {
        ...
        if (!isCli) {
            ...
            const knownAgents = agentList?.map(a => a.id).filter(Boolean) || [];
            const validateAgentList = (list, name) => {
                if (!list)
                    return;
                if (list.length === 0) {
        ...
        if (!isCli && !_telemetrySent) {
            ...
            (async () => {
                let memoryCount = 0;
                try {
                    memoryCount = (await store.stats()).totalCount;
                }
                catch { }
        ...
        api.registerGatewayMethod("memex.audit_logs", async ({ params, respond }) => {
            try {
                ...
                const run = await api.runtime.subagent.run({
                    ...
                    provider,
                    model,
                    extraSystemPrompt: "You are auditing memex plugin health. Use only the provided evidence. " +
            ...
            start: async () => {
                ...
                const runStartupChecks = async () => {
                    ...
                    try {
                        ...
                        api.logger.info(`memex: initialized successfully ` +
                ...
                if (config.sessionIndexing?.enabled) {
                    const runSessionIndex = async () => {
                        try {
                            ...
                            api.logger.info(`memex: session indexing complete — ` +
                    ...
                    try {
                        ...
                        if (allEntries.length > 0) {
                            ...
                            if (noiseEntries.length > 0) {
                                ...
                                api.logger.warn(`memex: store health — ${noiseEntries.length}/${allEntries.length} entries (${pct}%) are noise ` +
                    ...
                    if (searchStoreRef) {
                        try {
                            ...
                            if (orphanedContent > 0 || orphanedVectors > 0) {
                                api.logger.info(`memex: document cleanup — removed ${orphanedContent} orphaned content, ${orphanedVectors} orphaned vectors`);
                            }
                            ...
                            if (pending > 0) {
                                ...
                                api.logger.info(`memex: document recovery — embedded ${result.embedded} docs (${result.chunks} chunks)` +
...
function parsePluginConfig(value) {
    ...
    const rerankerRaw = cfg.reranker;
    ...
    return {
        ...
        sessionIndexing: typeof cfg.sessionIndexing === "object" && cfg.sessionIndexing !== null
            ? {
                enabled: cfg.sessionIndexing.enabled === true,
                agent: typeof cfg.sessionIndexing.agent === "string"
                    ? cfg.sessionIndexing.agent : undefined,
                scope: typeof cfg.sessionIndexing.scope === "string"
                    ? cfg.sessionIndexing.scope : undefined,
